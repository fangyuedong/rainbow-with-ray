...
opt = ray.remote(Optimizer).options(num_gpus=0.3).remote(dataloader, env_name, iter_steps=400, update_period=10000)
sche = Sched()
eps = 1
"""
class_name          method
...
def state_machine(tsk_dones, infos):
    ...
    for tsk_done, info in zip(tsk_dones, infos):
        if info.class_name == "DQN_Worker" and info.method == "__next__":
            ...
            if lmdb_op.len(buffer) > 10000 and not sche.have(opt, "__next__"):
                eps = 0.05
